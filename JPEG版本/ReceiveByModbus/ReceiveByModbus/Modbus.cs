...
namespace ReceiveByModbus
{
    public class Modbus
    {
        ...
        private ushort IsReadDataCorrect(byte count, int len)
        {
            /*
            ushort packetnum, size=0;
            int ret = BmpReceiveByte(ref receiveBuf, 0, len);
            if (ret < 6)
                return 0;
            packetnum = (ushort) (receiveBuf[3]*256+receiveBuf[2]);
            if(packetnum != count)
                return 0;
            //size = (ushort)(receiveBuf[5] * 256 + receiveBuf[4]);
            //if (size > ret)
            //    return 0;
            if (receiveBuf[ret-2] != 0xAA)
                return 0;
            if (receiveBuf[ret-1] != 0x55)
                return 0;
            return (ushort)ret;
             */
            return 0;
