...
extern u8 ComOutDown;
extern u8 ComInDataLen;
extern u8 COMHandleBUF[64];
extern u8 sensor_id;
extern u8 ComInBuf[64];
extern u16 COMHandleLEN;
u8 ComInDataOk = 0;

u8 timer_1ms_flag = 0;
u8 timer_10ms_flag = 0;
u8 timer_100ms_flag = 0;
u8 timer_1000ms_flag = 0;
u16 ComOutDelay	;
u16 USARTEndDelay;
u16 USARTSendFlag;
...
{ 
	...
	if(TIM3->SR&0X0001)//溢出中断
	{
		 ...
		 if(timer_100ms_flag >= 10)
		 {
			timer_100ms_flag = 0;
			//timer_1000ms_flag++;
			if((GPIOB->ODR&0X0800) != 0)
			{
				GPIOB->BSRR = 0x0000;	
				GPIOB->BRR = 0x0800;//GPIO_ResetBits(GPIOB, GPIO_Pin_11);
			}
			else
				GPIOB->BSRR = 0x0800;//GPIO_SetBits(GPIOB, GPIO_Pin_11);
		}
		...
		if(USARTEndDelay > 0)
		 ...
		 if(ComDelayCount > 0)
		 {
			 ...
			 if(ComDelayCount == 0)
			 {
					if(ComInBuf[0] == sensor_id)
					{
						ComInDataOk = 1;
						COMHandleLEN = ComInDataLen;
						for(i=0;i<COMHandleLEN;i++)
						{
								COMHandleBUF[i]= ComInBuf[i];
						}
					}
				ComInDataLen = 0;
			 }
