...
void Update_Outbuf(u16 size)
{
	 u16 i;
	 u8 *ptrData;
   u16 tempData;
	 u16 blk_num = size/UART_SND_BLK_DATA_SIZE; //RGB565

	 snd_buf_len = size + (blk_num+1) * 8;
	 for(i=0; i<blk_num; i++)
	 {
		   ptrData = (u8 *)(out_data+i*UART_SND_BLK_SIZE);
		   memcpy(ptrData, &sensor_id, 1);
			 memcpy(ptrData+2, (u8 *)&i, 2);
			 tempData = snd_buf_len;
	 ...
	 if(size != blk_num * UART_SND_BLK_SIZE)
	 {
		 //最后一个块
		 ptrData = (u8 *)(out_data+i*UART_SND_BLK_SIZE);
		 ...
		 //长度
		 //tempData = size - blk_num*UART_SND_BLK_DATA_SIZE;
		 tempData=snd_buf_len;
		 memcpy(ptrData+4, &tempData, 2);
		 memcpy(ptrData+6, raw_data+i*UART_SND_BLK_DATA_SIZE, tempData);
		 
		 tempData = 0x55AA;
		 memcpy(ptrData+size-blk_num * UART_SND_BLK_DATA_SIZE+6, &tempData, 2);
	 }
}
	...
	{			
 		...
 		for(y=0;y<76800;y++)//宽
		{
			  ...
			  //raw_data[2*y]= ((u8)color)& 0xFF;
				...
				red = (u8)((color >> 11) & 0x1f);
				green = (u8)((color >> 5) & 0x3f);
				blue = (u8)(color & 0x1f);
				red = (u8)((red << 3) | (red & 0x7));
				green = (u8)((green << 2) | (green & 0x3));
				blue = (u8)((blue << 3) | (blue & 0x7));			
			  out_data[3*y] = red;
				out_data[3*y+1] = green;
				out_data[3*y+2] = blue;
...
int main(void)
{
	...
	GPIOB->CRH&=0XFFF0FFFF;//IO状态设置
	GPIOB->CRH|=0X00010000;//IO状态设置
	...
	sensor_id = (u8)((*((u16 *)0X803f800))&0x00ff);
	if((sensor_id<0x01)||(sensor_id>0x1f))
{
		sensor_id=0x01;	
		//id_buf = 0x0000+(u16)sensor_id;
		FLASH_Unlock();
  ...
  OV7670_CS=0;				    		    
	...
	while(1)//识别每一帧	
	{
		...
		CassMem[172]=tempbrightness;
		CassMem[173]=tempcontrast;
		CassMem[174]=tempsaturation;
		...
		{  
			...
			jpeg_buf_len = jpeg_encoder(out_data, raw_data);
 			Update_Outbuf(jpeg_buf_len);
